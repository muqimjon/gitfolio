...
function attachReorder(el, i, arr, rerender) {
  el.draggable = true;
  el.ondragstart = (e) => { dragIdx = i; dragging = true; el.classList.add("dragging"); e.dataTransfer.effectAllowed = "move"; };
...
function attachSectionReorder(el) {
  el.draggable = true;
  el.ondragstart = (e) => { dragEl = el; dragging = true; el.classList.add("dragging"); e.dataTransfer.effectAllowed = "move"; };
...
function syncSuggest() {
  document.querySelectorAll("#stack_suggest .chip").forEach((el) => {
...
function syncSocialSuggest() {
  ...
  document.querySelectorAll("#social_suggest .chip").forEach((el) => {
    el.style.display = used.has(el.dataset.slug) ? "none" : "";
  });
}
function addSocial(str) {
  str = str.trim();
  if (!str) return;
  const i = str.indexOf(":");
...
function getColor(id) { return $(id).value.replace("#", ""); }

function checkedSections() {
  return [...document.querySelectorAll("#sections .chip.on")].map((c) => c.dataset.sec);
}
...
function buildParams() {
  ...
  const diff = (id, base) => getColor(id).toLowerCase() !== (base || "").toLowerCase();
...
function init() {
  ...
  $("stack_input").addEventListener("keydown", (e) => {
    if (e.key === "Enter" || e.key === ",") {
      e.preventDefault();
      e.target.value.split(",").forEach(addStack);
      e.target.value = "";
    }
  });

  $("social_input").addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      e.preventDefault();
      e.target.value.split(",").forEach(addSocial);
      e.target.value = "";
    }
  });
